...
def display_menu():
    ...
    print(f"                                                                         ")
    print(
        f"                                                      {colorama.Fore.BLUE}Version 1.0{colorama.Style.RESET_ALL}"
    )
    print(
    ...
    if choice == "1":
        ...
        if proxy_ask.lower() == "y":
            ...
            response = requests.get(
                "https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=50&country=all&ssl=all&anonymity=all"
            )
            proxies = [
                line.strip()
